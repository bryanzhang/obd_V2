...
def prune_columns(model, prune_ratio, index_dict, sum_es):
    for name, param in model.named_parameters():
        if not name.endswith(".weight"):
            continue
        weight = param
        es = sum_es[index_dict[name]]
        # 计算每行需要保留的权重数量
        num_weights_to_keep = max(1, int((1 - prune_ratio) * es.size(0)))
        for j in range(es.size(1)):
            column = es[:, j]
            threshold = torch.topk(column, num_weights_to_keep, largest=True).values.min()
            count = 0
            for i in range(es.size(0)):
                if count >= es.size(0) * prune_ratio:
                    break
                if es[i][j] < threshold:
                    with torch.no_grad():
                        weight[i][j] = 0.0
...
prune_columns(model, prune_ratio, index_dict, sum_es)
