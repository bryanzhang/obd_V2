            ...
            if first_grads[i].dim() == 2:
                for j in range(0, first_grads[i].size(0)):
                    ...
                    for k in range(0, first_grads[i].size(1)):
                        ...
                        with torch.no_grad():
                            #sum_es[i][j][k] += math.sqrt(abs(second_grad.float())) * abs(l[i][j][k].float())
                ...
                for j in range(0, first_grads[i].size(0)):
                    ...
                    with torch.no_grad():
                        #sum_es[i][j] += math.sqrt(abs(second_grad.float())) * abs(l[i][j].float())
...
root@iZuf6hns4egiw6oqxq2a96Z:~/obdV2#
root@iZuf6hns4egiw6oqxq2a96Z:~/obdV2#
root@iZuf6hns4egiw6oqxq2a96Z:~/obdV2# cat 3_mnist_obd_10000_samples.py
#! /bin/python3
